...
def test(model, loader, device, numz):
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            loss = 2*F.l1_loss(pred, x, reduction='mean') + 0.1*F.mse_loss(gender, gender_t) +0.6*F.mse_loss(bmi_t, bmi)+   0.6*F.mse_loss(wei_t, wei)+ 0.6*F.mse_loss(hei_t, hei) 
            loss_rec = F.mse_loss(gender, gender_t)
            total_loss_r += loss_rec.item()
            total_loss += loss.item()
            wei_t_r = wei_t*(max_bmi-min_bmi) + min_bmi
            wei_r = wei*(max_bmi-min_bmi) + min_bmi
            #if abs(bmi_t - bmi) < 0.1:
            acc_count = acc_count+(wei_t_r - wei_r)*(wei_t_r - wei_r)
...
def eval_error(model, test_loader, device, meshdata, out_dir, mesh, numz):
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            if  gender.cpu() > 0.90 and bmi.cpu() < 0.15 :
                p1 = z
                z1.append(z.unsqueeze(0))
            elif gender.cpu() < 0.10 and bmi.cpu() < 0.15 :
                p0 = z
...
#-------------------------------------
        ...
        #z_dimension = 7
            for i in range(10):
                ...
                #middle[:,int(numz*0.25):int(numz*1.00)] = z0_mean[:,int(numz*0.25):int(numz*1.00)]*0.5 + z1_mean[:,int(numz*0.25):int(numz*1.00)]*0.5
                #middle[:,int(numz*0.75):int(numz*1.00)] = z0_mean[:,int(numz*0.75):int(numz*1.00)]*0.5 + z1_mean[:,int(numz*0.75):int(numz*1.00)]*0.5
