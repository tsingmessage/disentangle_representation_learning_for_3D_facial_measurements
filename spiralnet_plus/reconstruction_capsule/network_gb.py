...
class AE(nn.Module):
    def __init__(self, in_channels, out_channels, latent_channels,
        ...
        self.numz = numz
        self.factor = 4
        self.step = 1/self.factor
        # latent generator
        self.latent_generator_g = torch.nn.Linear(1, int(numz/self.factor))
        self.latent_generator_b = torch.nn.Linear(1, int(numz/self.factor))
        self.latent_generator_w = torch.nn.Linear(1, int(numz/self.factor))
        self.latent_generator_h = torch.nn.Linear(1, int(numz/self.factor))
        # encoder
            ...
            if idx == 0:
                ...
                self.re_layers_b.append(
                    SpiralEnblock(out_channels[idx - 1], out_channels[idx],
                                  self.spiral_indices[idx]))
        self.re_layers_b.append(
            nn.Linear(int(numz/self.factor), 1))
        # regressor w
    ...
    def encoder(self, x):
        ...
        latent_z = my_drop(x)
        latent_b = latent_z[:,0:int(self.numz*self.step)]
        latent_w = latent_z[:,int(self.numz*self.step):int(self.numz*self.step*2)]
        latent_h = latent_z[:,int(self.numz*self.step*2):int(self.numz*self.step*3)]
        latent_g = latent_z[:,int(self.numz*self.step*3):int(self.numz*self.step*4)]
        latent_b = my_drop(latent_b)
        latent_w = my_drop(latent_w)
        latent_h = my_drop(latent_h)
        latent_g = my_drop(latent_g)
        bmi = self.re_layers_b[4](latent_b) #output layer for regressor
        wei = self.re_layers_w[4](latent_w) #output layer for regressor
        hei = self.re_layers_h[4](latent_h) #output layer for regressor        
        gender = self.cl_layers[4](latent_g) #output layer for regressor
        gender = torch.sigmoid(gender)
        return x, gender, x,bmi,wei,hei
