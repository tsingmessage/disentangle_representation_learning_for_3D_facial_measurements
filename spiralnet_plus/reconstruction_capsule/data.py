...
class ComaDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i_total, data_file in tqdm(enumerate(self.data_file)):
            ...
            index_gender = np.argwhere(d_idcg==int(idc_v[0]))
            index_bmi = np.argwhere(d_idcb==int(idc_v[0]))
            if len(index_gender) == 0 or len(index_bmi) == 0:
                ...
                label_age = 1*(label_age - min_age) / (max_age - min_age)            
                label_g = int(d_gender[index_gender])
                #print(label_y)
                if label_g == 999:
                    continue
                label_y = label_g - 1
                label_pos = torch.Tensor([label_bmi, label_age, label_height])
